...
import { Map, ClipboardList, Settings } from 'lucide-react'
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col h-screen bg-background overflow-hidden">
      {/* Main content */}
      <main className={cn('flex-1 overflow-hidden relative', isMapPage && 'overflow-hidden')}>
        {children}
      </main>

      {/* Bottom nav — hidden on map page (map has its own controls) */}
      {!isMapPage && (
