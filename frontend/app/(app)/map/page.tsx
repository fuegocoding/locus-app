...
import { Mic, Touchpad, Users, Radio, ChevronUp } from 'lucide-react'
  ...
  return (
    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end" onClick={onClose}>
      ...
      >
        {mode === 'convoy' ? (
          <>
            <h3 className="text-lg font-bold mb-4">You&apos;re in a convoy</h3>
    ...
    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end" onClick={onClose}>
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-2">
          <ActionButton label="Pin" icon="📍" onClick={() => { socketActions.pinUser(user.userId); onClose() }} neon />
          <ActionButton label="Mute" icon="🔇" onClick={() => { socketActions.muteUser(user.userId); onClose() }} />
          <ActionButton label="Block" icon="🚫" onClick={() => { socketActions.blockUser(user.userId); onClose() }} danger />
        </div>
      </div>
    </div>
  )
...
function ActionButton({ label, icon, onClick, neon, danger }: {
  label: string; icon: string; onClick: () => void; neon?: boolean; danger?: boolean
}) {
  return (
    <button
      onClick={onClick}
