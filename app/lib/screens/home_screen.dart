...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget _topBar(AppState s, ThemeData t) => Container(
    ...
    child: Row(children: [
      Container(width: 8, height: 8, decoration: BoxDecoration(shape: BoxShape.circle, color: s.mode == 'proximity' ? Colors.green : Colors.blue)), const SizedBox(width: 6),
  ...
  Widget _icon(IconData i, VoidCallback onTap) => GestureDetector(onTap: onTap, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: Icon(i, size: 18, color: Colors.white54)));
  ...
  Widget _btn(IconData i, String l, VoidCallback onTap) => GestureDetector(onTap: onTap, child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(i, size: 22, color: Colors.white70), const SizedBox(height: 2), Text(l, style: const TextStyle(fontSize: 9, color: Colors.white54))]));
  ...
  void _showConvoySheet() {
    ...
    showModalBottomSheet(context: context, builder: (_) => SafeArea(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [
      const Text('Create Convoy', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)), const SizedBox(height: 16),
      TextField(controller: c, decoration: InputDecoration(labelText: 'Convoy name', hintText: 'e.g. Road Trip Crew', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))), const SizedBox(height: 8),
      SizedBox(width: double.infinity, height: 52, child: ElevatedButton(onPressed: () { if (c.text.trim().isNotEmpty) { s.createConvoy(c.text.trim()); Navigator.pop(context); } }, child: const Text('Create'))),
      const SizedBox(height: 8), OutlinedButton(onPressed: () { Navigator.pop(context); _promptInviteCode(); }, child: const Text('Join with Invite Code')),
    ]))));
  }
